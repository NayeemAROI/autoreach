...
import { Send, MoreVertical, Archive, Tag, ExternalLink, Mail } from 'lucide-react'
...
export default function OwnerInboxPage() {
  ...
  const activeThread = mockInboxThread
    ...
    <div className="flex h-[calc(100vh-120px)] rounded-2xl bg-zinc-900/60 border border-zinc-800/60 overflow-hidden">
      ...
      <div className="w-80 border-r border-zinc-800/60 flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto">
          {threads.length === 0 && (
            <EmptyState icon={<Mail className="w-6 h-6" />} title="No messages" description="Your inbox is empty." className="py-12" />
          )}
          {threads.map(t => (
            <button
              key={t.id}
      ...
      <div className="flex-1 flex flex-col">
        {!activeThread ? (
          <EmptyState icon={<Mail className="w-7 h-7" />} title="No conversation selected" description="Select a thread from the left or wait for incoming messages." className="flex-1" />
        ) : (
          <>
      ...
      <div className="w-72 border-l border-zinc-800/60 p-4 hidden xl:block overflow-y-auto">
        ...
        {activeThread ? (
          ...
          <p className="text-xs text-zinc-600">No thread selected</p>
        )}
      </div>
    </div>
  )
